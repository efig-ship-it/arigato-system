...
if page == "Email Sender 📧":
    ...
    with col_up: up_ex = st.file_uploader("Mailing List (Excel)", type=['xlsx'])
    with col_due:
    ...
    if up_ex and uploaded_files:
        try:
            ...
            if missing and not st.toggle("🚨 Manual Override: I confirm files are correct", value=False):
...
# --- PAGE 2: ANALYTICS (📊 פיבוטים ופילטרים חזרו) ---
    ...
    if not df_raw.empty:
        ...
        st.write("### 📉 Collection Efficiency")
...
# --- PAGE 3: CONTROL (🔍 Multi הועבר למטה) ---
    ...
    if not df_raw.empty:
        ...
        if sel_label:
            ...
            with c_i1: entry = st.text_input("Internal Note:")
            with c_i2: rec_amt = st.number_input("Received ($):", value=float(row_data['received_amount']), key=f"ind_{sid}")
            with c_i3: nst = st.selectbox("Status:", ["Sent", "Paid", "Overdue", "In Dispute"], index=["Sent", "Paid", "Overdue", "In Dispute"].index(row_data['status']), key=f"st_{sid}")
            if st.button("Save Entry"):
                ...
                add_log_entry(sid, f"Logged: {f_st} | ${rec_amt:,.2f}")
        ...
        if st.button("🚀 Execute Batch Update", use_container_width=True):
            ...
            if not to_launch.empty:
                ...
                st.success(f"{len(to_launch)} updated."); time.sleep(1); st.rerun()
    else: st.info("No data.")
