...
# --- 2. UI CSS (Luxury Navy Dark Mode) ---
st.set_page_config(page_title="TMC Billing PRO", layout="wide")
st.markdown("""<style>
    .stApp { background-color: #0a192f; color: #e6f1ff; }
    section[data-testid="stSidebar"] { background-color: #112240 !important; border-right: 1px solid #233554; }
    div[data-testid="stMetric"] { 
        background-color: #112240; border-radius: 12px; border: 1px solid #233554; padding: 20px !important; 
    }
    div[data-testid="stMetricValue"] { color: #64ffda !important; font-size: 26px !important; }
    div[data-testid="stMetricLabel"] { color: #8892b0 !important; }
    .stButton>button {
        background-color: #64ffda; color: #0a192f; border-radius: 8px; font-weight: 700; width: 100%;
    }
    .tuesday-header { font-size: 32px; font-weight: 900; color: #64ffda; margin-bottom: 20px; }
    .alert-box { border-left: 5px solid #64ffda; margin-bottom: 20px; padding: 20px; background: #112240; border-radius: 8px; }
    .risk-box { border: 2px solid #f7567c; background-color: #2d1b24; padding: 15px; border-radius: 10px; color: #f7567c; }
    .detective-box { border: 2px solid #ed8936; background-color: #2d241b; padding: 15px; border-radius: 10px; color: #ed8936; }
    .log-box { background-color: #1d2d50; padding: 12px; border-radius: 8px; border-left: 4px solid #64ffda; margin-bottom: 8px; font-size: 13px; color: #ccd6f6; direction: ltr; }
...
if page == "Email Sender 📧":
    ...
    with col_due:
        st.markdown('<p style="font-weight:700; color:#8892b0;">BILLING PERIOD</p>', unsafe_allow_html=True)
    ...
    if up_ex:
        ...
        try:
            ...
            if not bad.empty:
                risk_cleared = st.checkbox("🚨 Risk: Confirm manual debt check", value=False)
                if not risk_cleared:
...
# --- PAGE 2: ANALYTICS DASHBOARD ---
    ...
    if not df_raw.empty:
        ...
        c1, c2 = st.columns(2)
        c1.markdown(f'<div class="alert-box" style="border-left-color:#f7567c;"><p style="color:#f7567c; font-weight:700;">🚨 TOTAL OVERDUE</p><h2>${risk_v:,.0f}</h2></div>', unsafe_allow_html=True)
        c2.markdown(f'<div class="alert-box" style="border-left-color:#64ffda;"><p style="color:#64ffda; font-weight:700;">🟢 7-DAY FORECAST</p><h2>${forecast_v:,.0f}</h2></div>', unsafe_allow_html=True)
        
        # 🔍 Global Filters
        st.divider(); st.subheader("Global Filters")
        f1, f2, f3 = st.columns(3)
        sel_c = f1.multiselect("Filter Companies", sorted(df_raw['company'].unique()))
        ...
        m1.metric("CEI Index", f"{cei:.1f}%"); m2.metric("Outstanding Balance", f"${df['balance'].sum():,.0f}"); m3.metric("Billed Total", f"${df['amount'].sum():,.0f}"); m4.metric("Reminded Debt", f"${df[df['status'] == 'Sent Reminder']['balance'].sum():,.0f}")

        # 📊 Charts
        st.divider(); g1, g2 = st.columns(2)
        with g1:
            st.subheader("Status Distribution")
            fig_p = px.pie(df.groupby('status')['amount'].sum().reset_index(), values='amount', names='status', color_discrete_sequence=['#64ffda', '#f7567c', '#00d2ff', '#ed8936'])
            fig_p.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="#ccd6f6")
        ...
        with g2:
            ...
            if not debtors[debtors['balance']>0].empty:
                fig_b = px.bar(debtors[debtors['balance']>0], x='balance', y='company', orientation='h', color_discrete_sequence=['#f7567c'])
                fig_b.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="#ccd6f6")
                st.plotly_chart(fig_b, use_container_width=True)

        # 📑 Pivot Tables
        st.divider(); st.subheader("Pivot Analysis")
        st.dataframe(df.pivot_table(index='company', columns='status', values='amount', aggfunc='sum', fill_value=0), use_container_width=True)
        pc1, pc2 = st.columns(2)
        with pc1: st.write("**Monthly Summary**"); st.dataframe(df.pivot_table(index='month_sent', values='amount', aggfunc='sum'), use_container_width=True)
        with pc2: 
            st.write("**Avg Payment Speed**")
            spd = df[df['days_to_pay'].notna()]
            if not spd.empty: st.dataframe(spd.groupby('company')['days_to_pay'].mean().reset_index(), use_container_width=True)

        st.divider(); st.write("**Billed vs Received (Efficiency Graph)**")
        ...
        st.vega_lite_chart(pd.concat([b, r]), {'mark': {'type': 'bar', 'width': 20, 'cornerRadiusTopLeft': 3}, 'encoding': {'x': {'field': 'due_date_str', 'type': 'nominal', 'title': 'Due Date'}, 'y': {'field': 'Val', 'type': 'quantitative'}, 'xOffset': {'field': 'Type'}, 'color': {'field': 'Type', 'type': 'nominal', 'scale': {'range': ['#64ffda', '#1d2d50']}}}}, use_container_width=True)

# --- PAGE 3: UPCOMING ALERTS (T-7 Pulse) ---
elif page == "Upcoming Alerts 🔔":
    st.title("Proactive T-7 Pulse Board")
    df_raw = get_cloud_history()
    if not df_raw.empty:
        target = date.today() + timedelta(days=7)
        upcoming = df_raw[(df_raw['due_date_obj'] == target) & (df_raw['status'] != 'Paid')].copy()
        if upcoming.empty: st.info(f"All quiet for {target.strftime('%Y-%m-%d')}.")
        else:
                ...
                if not sel[sel['Select']].empty and mf:
                    ...
                    for i, row in sel[sel['Select']].iterrows():
                        ...
                        if target_email:
                            ...
                            server.send_message(msg); add_log_entry(row['id'], "Sent proactive T-7 reminder.")
                    server.quit(); st.success("Pulse reminders sent!"); st.rerun()

# --- PAGE 4: COLLECTIONS CONTROL (Full Batch + Filters) ---
elif page == "Collections Control 🔍":
    st.title("Operations & Audit Control")
    df_raw = get_cloud_history()
    if not df_raw.empty:
        cf1, cf2 = st.columns(2)
        c_sel = cf1.multiselect("Search Companies", sorted(df_raw['company'].unique()))
        c_due = cf2.date_input("Filter Dates", value=(df_raw['due_date_obj'].min(), df_raw['due_date_obj'].max()))
        f_df = df_raw.copy()
        ...
        st.dataframe(f_df[['id', 'company', 'date_sent_str', 'due_date', 'amount', 'received_amount', 'status']]
                     .style.applymap(lambda x: 'background-color: #2d1b24; color: #f7567c; font-weight: bold;' if x=='Overdue' else '', subset=['status']), use_container_width=True, hide_index=True)
        ...
        if sel_l:
            ...
            with ci2: rec = st.number_input("Received Amount ($):", value=float(row_data['received_amount']), key=f"rec_{sid}")
            ...
            if st.button("UPDATE RECORD"):
                ...
                add_log_entry(sid, f"Update: {f_st} | ${rec:,.0f}"); st.success("Updated."); time.sleep(0.5); st.rerun()

        st.divider(); st.subheader("⚡ Batch Execute Launch (Multi-Edit)")
        bulk = f_sorted[['id', 'company', 'due_date', 'amount', 'received_amount']].copy()
        bulk['Select'] = False
        sel_bulk = st.data_editor(bulk[['Select', 'company', 'due_date', 'amount', 'received_amount', 'id']], column_config={"Select": st.column_config.CheckboxColumn("V", default=False), "id": None}, hide_index=True, use_container_width=True)
        if st.button("🚀 EXECUTE BATCH UPDATE"):
            for i, row in sel_bulk[sel_bulk['Select']].iterrows():
                ...
                add_log_entry(row['id'], f"Batch Update: {f_st}"); st.success("Batch Complete."); time.sleep(1); st.rerun()

# --- PAGE 5: REMINDERS MANAGER (Debt Recovery) ---
elif page == "Reminders Manager 🚨":
    st.title("Debt Recovery Board")
    mf = st.file_uploader("Upload Mailing List", type=['xlsx'])
    df_raw = get_cloud_history()
    if not df_raw.empty:
        df_raw['balance'] = df_raw['amount'] - df_raw['received_amount']
        unpaid = df_raw[df_raw['balance'] > 0].copy()
        if unpaid.empty: st.success("All debts cleared!")
        else:
            unpaid['Select'] = False
            sel_d = st.data_editor(unpaid[['Select', 'company', 'due_date', 'balance', 'id']], hide_index=True, use_container_width=True)
            d1, d2 = st.columns(2); mu = d1.text_input("Gmail Account"); mp = d2.text_input("Pass", type="password")
            if st.button("🚀 SEND RECOVERY ALERTS"):
                if not sel_d[sel_d['Select']].empty and mf:
                    ...
                    for i, row in sel_d[sel_d['Select']].iterrows():
                        ...
                        if target:
                            msg = MIMEMultipart(); msg['Subject'] = f"URGENT: Outstanding Debt - {row['company']}"; msg['To'] = target
                            msg.attach(MIMEText(f"Dear {row['company']}, you have a debt of ${row['balance']:,.2f}.", 'plain'))
                            server.send_message(msg); add_log_entry(row['id'], f"Recovery Alert sent to {target}"); supabase.table("billing_history").update({"status": "Sent Reminder"}).eq("id", row['id']).execute()
