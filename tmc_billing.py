...
def clean_amount(val):
    try: return float(re.sub(r'[^\d.]', '', str(val))) if pd.notna(val) else 0.0
    except: return 0.0
...
if page == "Email Sender 📧":
    ...
    if up_ex:
        ...
        try:
            ...
            if not bad.empty:
                ...
                if not risk_ack:
                    st.markdown(f'<div class="risk-box">⚠️ <b>Risk Alert:</b> {len(bad)} companies are 30+ days overdue.</div>', unsafe_allow_html=True)
                    risk_cleared = False
    ...
    if st.button("🚀 Start Dispatch", use_container_width=True, disabled=not (up_ex and uploaded_files and risk_cleared and detective_cleared)):
        try:
            df_master = pd.read_excel(up_ex).dropna(how='all'); due_col = [c for c in df_master.columns if 'due' in c.lower()]
...
# --- PAGE 2: ANALYTICS (TREND CHART RESTORED) ---
    ...
    if not df_raw.empty:
        ...
        st.write("**Company Status Breakdown**"); st.dataframe(df.pivot_table(index='company', columns='status', values='amount', aggfunc='sum', fill_value=0).style.format("${:,.0f}"), use_container_width=True)
        ...
        st.divider(); st.subheader("Efficiency: Billed vs Received (Trend)")
        trend = df.groupby('due_date_obj')[['amount', 'received_amount']].sum().cumsum().reset_index()
        fig_t = px.line(trend, x='due_date_obj', y=['amount', 'received_amount'], labels={'value': 'Total Amount ($)', 'due_date_obj': 'Due Date'}, markers=True, color_discrete_map={'amount': '#003366', 'received_amount': '#64ffda'})
...
# --- PAGE 4: COLLECTIONS CONTROL (BATCH RESTORED) ---
...
# --- PAGE 5: REMINDERS MANAGER (🚨 SIREN + LOCK) ---
