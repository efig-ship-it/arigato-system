...
if page == "Email Sender":
    ...
    if st.button("🚀 Start Bulk Sending", use_container_width=True):
        try:
            ...
            with st.spinner(""):
                st.markdown("""<div class="suitcase-container"><svg width="50" height="50" viewBox="0 0 24 24" fill="#8B4513"><path d="M17,6H16V5c0-1.1-0.9-2-2-2h-4C8.9,3,8,3.9,8,5v1H7C5.9,6,5,6.9,5,8v11c0,1.1,0.9,2,2,2h10c1.1,0,2-0.9,2-2V8 C19,6.9,18.1,6,17,6z M10,5h4v1h-4V5z M17,19H7V8h10V19z"/></svg><p style='color: #8B4513;'>Invoices are traveling...</p></div>""", unsafe_allow_html=True)
                for i, row in df_master.iterrows():
                    ...
                    if emails and company_files:
                        ...
                        supabase.table("billing_history").insert({"date": it, "company": company, "amount": total_amt, "status": "Sent", "due_date": due_val, "sender": user_mail}).execute()
...
# --- PAGE 2: ANALYTICS (📊 גרף עמודות צמודות) ---
    ...
    if not df_raw.empty:
        ...
        m1.metric("Total Billed", f"${tb:,.2f}"); m2.metric("Actually Received", f"${tp:,.2f}"); m3.metric("Outstanding", f"${tb-tp:,.2f}")
...
# --- PAGE 3: CONTROL (🔍 תיעוד והיסטוריה למטה) ---
                ...
                if st.button("💾 Save Changes"):
                    for i, row in edited_df.iterrows():
                        ...
                        if row['status'] != old_row['status']:
                            add_log_entry(row['id'], f"Update: {row['status']} on {date.today()}")
        ...
        if sel_label:
            ...
            if st.button("שמור"):
                ...
                if nst != row_data['status']:
                    add_log_entry(sid, f"Status to {nst}")
