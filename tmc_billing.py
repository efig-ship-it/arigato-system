...
# --- PAGE 2: ANALYTICS (📊 פילטרים + מדדים + פיבוטים) ---
    ...
    if not df_raw.empty:
        ...
        m1.metric("Billed", f"${tb:,.0f}")
        m2.metric("Received", f"${tr:,.0f}")
        m3.metric("Outstanding", f"${tb-tr:,.0f}")
        m4.metric("Reminded", f"${rem_total:,.0f}")
                    ...
                    try:
                        ...
                        for i, row in to_send.iterrows():
                            ...
                            if target:
                                ...
                                server.send_message(msg)
                                add_log_entry(row['id'], f"Reminder sent to {target}")
