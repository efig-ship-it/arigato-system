...
# --- PAGE 4: REMINDERS MANAGER 🚨 ---
    ...
    if not df_raw.empty:
        ...
        if unpaid_df.empty: st.success("All invoices fully paid!")
            ...
            d_sc1, d_sc2 = st.columns(2); d_mail = d_sc1.text_input("Sender Gmail"); d_pass = d_sc2.text_input("Sender Password", type="password")
            if st.button("🚀 Send Reminders & Update Status", use_container_width=True):
