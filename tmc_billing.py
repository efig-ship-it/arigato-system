...
# --- 2. CSS & Design (🎨 עיצוב ושפה - ללא שינוי) ---
...
# --- PAGE 1: EMAIL SENDER (📧 שליחת מיילים - כולל SUCCESS ומוזיקה) ---
if page == "Email Sender":
    ...
    if st.button("🚀 Start Bulk Sending", use_container_width=True, disabled=not allow_sending):
        if not up_ex or not user_mail:
            st.error("Missing credentials or file.")
        else:
            try:
                    ...
                    for i, row in df_master.iterrows():
                        ...
                        if emails and company_files:
                            ...
                            supabase.table("billing_history").insert({
                                "date": it.strftime("%d/%m/%Y %H:%M"), "company": company, "amount": clean_amount(row[amount_col]) if amount_col else 0.0,
                                "status": "Sent", "currency": "$", "sender": user_mail, "due_date": f"{sel_y}-{months.index(sel_m)+1:02d}-15"
                            }).execute()
                server.quit()
...
# --- PAGE 2: ANALYTICS (📊 דשבורד - חזרה למבנה המקורי לפי החוזה) ---
    ...
    if not df.empty:
        ...
        dr_dash = f2.date_input("Filter Dates", value=[min_date, max_date])
        if isinstance(dr_dash, list) and len(dr_dash) == 2:
            f_df = f_df[(f_df['date_obj'] >= dr_dash[0]) & (f_df['date_obj'] <= dr_dash[1])]
...
# --- PAGE 3: CONTROL (🔍 לוח בקרה) ---
    ...
    if not df.empty:
        ...
        edited_df = st.data_editor(f_df[display_cols].style.map(color_status, subset=['status']),
