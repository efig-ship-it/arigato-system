...
def get_cloud_history():
    try:
        ...
        if not df.empty:
            ...
            # המרה בטוחה לפורמט תאריך של פייתון להשוואה בהמשך
            df['due_date_obj'] = pd.to_datetime(df['due_date'], errors='coerce').dt.date
            return df
    except Exception as e:
        st.error(f"Error connecting to Cloud History: {e}")
    return pd.DataFrame()
                ...
                if email and (pdf_files or comp_excels):
                    with st.spinner(f"Sending to {comp}..."):
                        ...
                        for f in (pdf_files + comp_excels):
                            f.seek(0) # מוודא שהקובץ נקרא במלואו לצורך הצירוף
