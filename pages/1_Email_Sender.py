...
def init_connection():
    try:
        # ניקוי אגרסיבי של הכתובות כדי למנוע שגיאות DNS (Name or service not known)
        u = st.secrets["SUPABASE_URL"].strip().strip('"').strip("'")
        # הסרת לוכסן סופי אם קיים בטעות
        if u.endswith('/'):
            u = u[:-1]
        ...
        st.info("Check your Secrets for spaces or slashes at the end of the URL.")
        st.stop()
...
def get_cloud_history():
    try:
        ...
        if not df.empty:
            ...
            # המרה בטוחה לתאריך לצורך השוואת סיכונים
            df['due_date_obj'] = pd.to_datetime(df['due_date'], errors='coerce').dt.date
            return df
    except Exception as e:
        st.error(f"⚠️ Could not fetch history from Cloud: {e}")
    return pd.DataFrame()
...
uploaded_files = st.file_uploader("2. Drop Company Files (PDF & Excel)", accept_multiple_files=True)
...
if up_mailing:
    try:
        ...
        # זיהוי עמודות גמיש (עברית/אנגלית)
        comp_col = next((c for c in mailing_data.columns if 'comp' in c or 'חברה' in c or 'לקוח' in c), mailing_data.columns[0])
        email_col = next((c for c in mailing_data.columns if 'mail' in c or 'מייל' in c or 'email' in c), None)
        day_col = next((c for c in mailing_data.columns if 'day' in c or 'יום' in c or 'due' in c), None)

        if not email_col:
            st.error("❌ לא נמצאה עמודת אימייל בקובץ. וודא שאחת הכותרות מכילה את המילה Email או מייל.")
        else:
            ...
            if uploaded_files:
                ...
                if missing:
                    st.markdown(f'<div class="detective-box">🕵️‍♂️ <b>Detective:</b> חסרים קבצים עבור: {", ".join(missing)}</div>', unsafe_allow_html=True)
                    det_ack = st.checkbox("אני מאשר שליחה למרות הקבצים החסרים")
            ...
            if not df_history.empty:
                ...
                if not bad_rows.empty:
                    st.markdown(f'<div class="risk-box">🚨 <b>Risk:</b> נמצאו חובות פתוחים עבור חלק מהחברות ברשימה.</div>', unsafe_allow_html=True)
                    risk_ack = st.checkbox("אני מאשר שליחת חשבוניות לבעלי חוב")
...
if st.button("🚀 Start Dispatch", use_container_width=True, disabled=not (up_mailing and uploaded_files and risk_cleared and detective_cleared)):
    try:
            ...
            for _, row in mailing_data.iterrows():
                ...
                if email and (pdf_files or comp_excels):
                    with st.spinner(f"Sending to {comp}..."):
                        ...
                        body = f"Hello {comp} Team,\n\nPlease find attached the invoices for {sel_m} {sel_y}.\nTotal amount: ₪{total_sum:,.2f}\n\nRegards,"
        ...
        server.quit()
        st.balloons()
        st.success("All Invoices Sent and Cloud Updated!")
        time.sleep(2)
        st.rerun()
        
    except Exception as e:
        st.error(f"Dispatch Error: {e}")
        st.info("If the error is 'Name or service not known', it's likely a temporary DNS issue or a typo in the Supabase URL in Secrets.")
