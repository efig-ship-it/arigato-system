...
# ייבוא פונקציות הליבה מקובץ ה-app הראשי
...
# --- CENTRAL LAYOUT (ריכוז התוכן לאורך) ---
...
with center_col:
    ...
    if up_ex:
        ...
        try:
            ...
            # --- DETECTIVE ALERT (בדיקת התאמה בין אקסל לקבצים) ---
    ...
    if st.button("🚀 Start Dispatch", use_container_width=True, disabled=not can_send):
        try:
            ...
            with status_placeholder.container():
                ...
                with st.spinner("Tuesday is dispatching your invoices..."):
                    for _, row in df_master.iterrows():
                        ...
                        # התאמת קבצים וחילוץ סכום אוטומטי
            ...
            st.balloons()
            st.success("SUCCESS: All invoices dispatched and logged in the cloud.")
            time.sleep(2)
            st.rerun()
    ...
    if not can_send and up_ex:
        st.caption("🔒 Complete all steps and resolve alerts to unlock dispatch.")
