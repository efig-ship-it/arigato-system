...
if not df_raw.empty:
    ...
    if sel_l:
        ...
        # הצגת הערות בתוך Expander מעוצב
                    ...
                    if line.strip():
                        st.markdown(f"""
                            <div class="note-bubble">
                                <span class="note-time">🕒 Log Entry</span>
                                {line}
                            </div>
                        """, unsafe_allow_html=True)
            else:
                st.info("No notes documented for this record yet.")
        
        ci1, ci2, ci3 = st.columns([2, 1, 1])
        with ci1: ent = st.text_input("Add New Note:")
        with ci2: rec = st.number_input("Received:", value=float(row_data['received_amount']), key=f"r_{sid}")
        with ci3: nst = st.selectbox("Status:", ["Sent", "Paid", "Overdue", "In Dispute", "Sent Reminder", "Partial"], index=["Sent", "Paid", "Overdue", "In Dispute", "Sent Reminder", "Partial"].index(row_data['status']) if row_data['status'] in ["Sent", "Paid", "Overdue", "In Dispute", "Sent Reminder", "Partial"] else 0, key=f"s_{sid}")
    ...
    if st.button("🚀 Execute Batch Update", use_container_width=True):
        ...
        if not rows_to_update.empty:
            for _, row in rows_to_update.iterrows():
                ...
                # השארתי את הלוגיקה החכמה שלך בדיוק כפי שהייתה
