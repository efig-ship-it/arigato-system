...
with c2:
    ...
    if not df[df['balance'] > 0].empty:
        ...
        fig_tree.update_layout(height=250, margin=dict(t=30, b=0, l=0, r=0))
        st.plotly_chart(fig_tree, use_container_width=True)
    else: st.info("No Debt to show")

with c3:
    # 3. Monthly Trend Line
    df['m'] = pd.to_datetime(df['date_dt']).dt.strftime('%b')
    trend = df.groupby('m').agg({'amount':'sum', 'received_amount':'sum'}).reset_index()
    fig_line = go.Figure()
    ...
    fig_line.update_layout(height=250, title="Collection Over Time", margin=dict(t=30, b=0, l=0, r=0), template="simple_white")
